...
// This class is taken from the Avalonia unit tests project and modified
// Original source code can be found here: https://github.com/AvaloniaUI/Avalonia/blob/a5f5b8f67ec0cd2fdde444ea7ba0c8cdd6d8d048/tests/Avalonia.UnitTests/MouseTestHelper.cs#L7
// It, like the rest of the Avalonia project, is MIT licensed and copyright AvaloniaUI OÜ
// The full text of the license is available here: https://github.com/AvaloniaUI/Avalonia/blob/master/licence.md
public class MouseTestHelper
{
        ...
        Point position = default, KeyModifiers modifiers = default, int clickCount = 1)
    {
        ...
        else
        {
            ...
            source.RaiseEvent(new PointerPressedEventArgs(source, _pointer, GetRoot(target), position, Timestamp(),
                props,
    ...
    {
        target.RaiseEvent(new PointerEventArgs(InputElement.PointerMovedEvent, source, _pointer, GetRoot(target),
            position,
            Timestamp(), new PointerPointProperties((RawInputModifiers)_pressedButtons, PointerUpdateKind.Other),
            modifiers));
    }
    ...
    {
        target.RaiseEvent(new PointerEventArgs(InputElement.PointerEnteredEvent, target, _pointer, (Visual)target,
            default,
            Timestamp(), new PointerPointProperties((RawInputModifiers)_pressedButtons, PointerUpdateKind.Other),
            KeyModifiers.None));
    }
    ...
    {
        target.RaiseEvent(new PointerEventArgs(InputElement.PointerExitedEvent, target, _pointer, (Visual)target,
            default,
            Timestamp(), new PointerPointProperties((RawInputModifiers)_pressedButtons, PointerUpdateKind.Other),
            KeyModifiers.None));
    }
