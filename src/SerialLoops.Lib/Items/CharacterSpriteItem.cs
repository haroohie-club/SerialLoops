...
public class CharacterSpriteItem(CharacterSprite sprite, CharacterDataFile chrdata, Project project, ILogger log)
    : Item(
        $"SPR_{project.Characters[(int)sprite.Character].Name}_{chrdata.Sprites.IndexOf(sprite):D3}{(sprite.IsLarge ? "_L" : "")}",
        ItemType.Character_Sprite), IPreviewableGraphic
{
    ...
    {
        return Sprite.GetClosedMouthAnimation(project.MessInfo, Graphics.BodyLayout, Graphics.BodyTextures,
            Graphics.EyeAnimation, Graphics.EyeTexture, Graphics.MouthAnimation, Graphics.MouthTexture);
    }

    public List<(SKBitmap Frame, int Timing)> GetLipFlapAnimation(Project project)
    {
        return Sprite.GetLipFlapAnimation(project.MessInfo, Graphics.BodyLayout, Graphics.BodyTextures,
            Graphics.EyeAnimation, Graphics.EyeTexture, Graphics.MouthAnimation, Graphics.MouthTexture);
    }

    public SKBitmap GetBaseLayout()
    {
        return Graphics.BodyLayout.GetLayout(Graphics.BodyTextures, 0, Graphics.BodyLayout.LayoutEntries.Count,
            darkMode: false, preprocessedList: true).bitmap;
    }

    public List<(SKBitmap Frame, short Timing)> GetEyeFrames()
    {
        return Graphics.EyeAnimation.GetAnimationFrames(Graphics.EyeTexture).Select(g => g.GetImage())
            .Zip(Graphics.EyeAnimation.AnimationEntries.Select(a => ((FrameAnimationEntry)a).Time)).ToList();
    }

    public List<(SKBitmap Frame, short Timing)> GetMouthFrames()
    {
        return Graphics.MouthAnimation.GetAnimationFrames(Graphics.MouthTexture).Select(g => g.GetImage()).ToList()
            .Zip(Graphics.MouthAnimation.AnimationEntries.Select(a => ((FrameAnimationEntry)a).Time)).ToList();
    }

    public void SetSprite(SKBitmap layoutBitmap, List<(SKBitmap Frame, short Time)> eyeFramesAndTimings,
        List<(SKBitmap Frame, short Time)> mouthFramesAndTimings, short eyeX, short eyeY, short mouthX, short mouthY)
    {
        ...
        for (short i = 0; i < 3; i++)
        {
            ...
            for (short y = 0; y < texture.Height; y += 32)
            {
                for (short x = 0; x < texture.Width; x += 32)
                {
                    ...
                    textureCanvas.DrawBitmap(layoutBitmap, src, dst, SKSamplingOptions.Default);
    ...
    {
        GraphicsFile newMouthTexture =
            Graphics.MouthAnimation.SetFrameAnimationAndGetTexture(framesAndTimings, palette);
...
public class CharacterSpriteGraphics(CharacterSprite sprite, ArchiveFile<GraphicsFile> grp)
{
    ...
    [
        grp.GetFileByIndex(sprite.TextureIndex1), grp.GetFileByIndex(sprite.TextureIndex2),
        grp.GetFileByIndex(sprite.TextureIndex3)
    ];
    ...
    {
        IO.WriteStringFile(Path.Combine("assets", "graphics", $"{BodyLayout.Index:X3}.lay"),
            JsonSerializer.Serialize(BodyLayout.LayoutEntries, Project.SERIALIZER_OPTIONS), project, log);
        ...
        foreach (GraphicsFile bodyTexture in BodyTextures)
        {
            ...
            IO.WriteBinaryFile(Path.Combine("assets", "graphics", $"{bodyTexture.Index:X3}.png"),
                bodyTextureStream.ToArray(), project, log);
            IO.WriteStringFile(Path.Combine("assets", "graphics", $"{bodyTexture.Index:X3}.gi"),
                bodyTexture.GetGraphicInfoFile(), project, log);
        }

        IO.WriteBinaryFile(Path.Combine("assets", "graphics", $"{EyeAnimation.Index:X3}.bna"), EyeAnimation.GetBytes(),
            project, log);
        ...
        IO.WriteBinaryFile(Path.Combine("assets", "graphics", $"{EyeTexture.Index:X3}.png"), eyeTextureStream.ToArray(),
            project, log);
        IO.WriteStringFile(Path.Combine("assets", "graphics", $"{EyeTexture.Index:X3}.gi"),
            EyeTexture.GetGraphicInfoFile(), project, log);

        IO.WriteBinaryFile(Path.Combine("assets", "graphics", $"{MouthAnimation.Index:X3}.bna"),
            MouthAnimation.GetBytes(), project, log);
        ...
        IO.WriteBinaryFile(Path.Combine("assets", "graphics", $"{MouthTexture.Index:X3}.png"),
            mouthTextureStream.ToArray(), project, log);
        IO.WriteStringFile(Path.Combine("assets", "graphics", $"{MouthTexture.Index:X3}.gi"),
            MouthTexture.GetGraphicInfoFile(), project, log);
    }
}
