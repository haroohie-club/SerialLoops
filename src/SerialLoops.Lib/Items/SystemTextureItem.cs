...
public class SystemTextureItem : Item
{
    ...
    private const string MainPalette =
    ...
    public SKBitmap GetTexture()
    {
        ...
        else
        {
            ...
            for (int y = 0; y < tileBitmap.Height; y += SysTex.TileHeight)
            {
                ...
                {
                    SKRect crop = new(0, currentTile * SysTex.TileHeight, SysTex.TileWidth,
                        (currentTile + 1) * SysTex.TileHeight);
    ...
    public void SetTexture(SKBitmap bitmap, bool replacePalette, ILogger log)
    {
        if (replacePalette)
        {
            List<SKColor> replacedPalette = Helpers.GetPaletteFromImage(bitmap, 255, log);
        ...
        else
        {
            ...
            for (int y = 0; y < bitmap.Height; y += SysTex.TileHeight)
            {
                for (int x = 0; x < bitmap.Width; x += SysTex.TileWidth)
                {
                    ...
                    SKRect dest = new(0, currentTile * SysTex.TileHeight, SysTex.TileWidth,
                        (currentTile + 1) * SysTex.TileHeight);
                    tileCanvas.DrawBitmap(bitmap, crop, dest, SKSamplingOptions.Default);
                    currentTile++;
                }
            }
    ...
    public void Write(Project project, ILogger log)
    {
        ...
        IO.WriteBinaryFile(Path.Combine("assets", "graphics", $"{Grp.Index:X3}.png"), grpStream.ToArray(), project,
            log);
        IO.WriteStringFile(Path.Combine("assets", "graphics", $"{Grp.Index:X3}.gi"), Grp.GetGraphicInfoFile(), project,
            log);
    }
    ...
    {
        return MainPalette.Equals(string.Join(',', Grp.Palette.Select(c => c.ToString())),
            StringComparison.OrdinalIgnoreCase);
    }

    public bool UsesExtraPalette()
    {
        return ExtraPalette.Equals(string.Join(',', Grp.Palette.Select(c => c.ToString())),
            StringComparison.OrdinalIgnoreCase);
    }

    public bool UsesChessPalette()
    {
        return ChessPalette.Equals(string.Join(',', Grp.Palette.Select(c => c.ToString())),
            StringComparison.OrdinalIgnoreCase);
    }
}
