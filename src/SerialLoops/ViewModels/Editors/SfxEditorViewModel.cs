...
public class SfxEditorViewModel : EditorViewModel
{
    ...
    {
        _player.Stop();
        IStorageFile wavFile = await GuiExtensions.ShowSaveFilePickerAsync(Window.Window, Strings.SfxEditorExportButton, [new(Strings.WAV_File) { Patterns = ["*.wav"] }], $"{Sfx.DisplayName}.wav");
